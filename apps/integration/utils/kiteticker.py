...
class KiteTickerClientProtocol(WebSocketClientProtocol):
    ...
    def onClose(self, was_clean, code, reason):  # noqa
        ...
        if not was_clean and self.factory.on_error:
    ...
    def onPong(self, response):  # noqa
        ...
        if self._last_pong_time and self.factory.debug:
            log.debug(
                f"last pong was {time.time() - self._last_pong_time} seconds back."
            )
    ...
    def _loop_ping(self):  # noqa
        ...
        if self.factory.debug:
            ...
            if self._last_ping_time:
                log.debug(
                    f"last ping was {time.time() - self._last_ping_time} seconds back."
                )
...
class KiteTickerClientFactory(WebSocketClientFactory, ReconnectingClientFactory):
    ...
    def clientConnectionLost(self, connector, reason):  # noqa
        ...
        if self.retries > 0 and self.on_reconnect:
            self.on_reconnect(self.retries)
...
class KiteTicker(object):
    ...
    def _user_agent(self):
        return ("kiteconnect-python/").capitalize() + "4.2.0"
    ...
    def _parse_binary(self, bin):  # sourcery skip: low-code-quality
                        ...
                        depth["sell" if i >= 5 else "buy"].append(
                            {
                                ...
                                "price": self._unpack_int(packet, p + 4, p + 8)
                                / divisor,
...
class KiteExtTicker(KiteTicker):
    def __init__(
        ...
        enctoken = quote(enctoken)
        self.socket_url = f"{root}?api_key={api_key}&user_id={user_id}&enctoken={enctoken}&user-agent={user_agent}&version={version}"
